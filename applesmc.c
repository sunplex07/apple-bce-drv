...
/*
 * T2 (iBridge) suspend/resume SMC keys.
 * Verified from macOS assembly: setSMCToSleepStatic at 0xffffff8001c284d0.
 *
 * MSDW: Main Sleep/Down/Wake — tells T2 the host is sleeping (0) or awake (1).
 * WKTP: Wake Type — configures which wake sources T2/EC monitors.
 * NTOK: Notification Token — disables SMC attention interrupts before sleep.
 * CLKL: Clock Lock — invalidates host-T2 clock sync before sleep.
 * OSWD: OS Watchdog — T2 reboots if this isn't periodically written.
 */
#define T2_KEY_MSDW		"MSDW"
#define T2_KEY_WKTP		"WKTP"
#define T2_KEY_NTOK		"NTOK"
#define T2_KEY_CLKL		"CLKL"
#define T2_KEY_OSWD		"OSWD"
...
struct applesmc_registers {
	...
	bool has_t2_sleep;		/* has T2 sleep keys (MSDW) */
...
static int applesmc_init_smcreg_try(struct applesmc_device *smc)
{
	...
	if (ret)
		return ret;
	ret = applesmc_has_key(smc, T2_KEY_NTOK, &s->has_t2_sleep);
	if (ret)
		return ret;
	...
	pr_info("key=%d fan=%d temp=%d index=%d acc=%d lux=%d kbd=%d t2sleep=%d\n",
...
static struct applesmc_device *global_applesmc;
...
/*
 * T2 suspend: notify T2 that the host is going to sleep.
 * Runs as .suspend_late, after all device drivers have suspended.
 */
static int applesmc_pm_suspend_late(struct device *dev)
{
	...
	if (!smc->reg.has_t2_sleep)
		return 0;

	dev_info(smc->ldev, "T2 sleep: entering suspend_late\n");

	/* NTOK=0x01 before sleep — confirmed working on real T2 hardware */
	val_u8 = 0x01;
	ret = applesmc_write_key(smc, T2_KEY_NTOK, &val_u8, 1);
	if (ret)
		dev_err(smc->ldev, "T2 sleep: writing NTOK=0x01 failed: %d\n", ret);
	else
		dev_info(smc->ldev, "T2 sleep: NTOK=0x01\n");

	/* Configure T2/EC wake sources for full sleep */
	val_u8 = 0x02;
	ret = applesmc_write_key(smc, T2_KEY_WKTP, &val_u8, 1);
	if (ret)
		dev_err(smc->ldev, "T2 sleep: writing WKTP=0x02 failed: %d\n", ret);
	else
		dev_info(smc->ldev, "T2 sleep: WKTP=0x02 (full sleep wake sources)\n");

	/* Invalidate host-T2 clock sync (hex_, 6 bytes) */
	memset(clkl_buf, 0xFF, 6);
	ret = applesmc_write_key(smc, T2_KEY_CLKL, clkl_buf, 6);
	if (ret)
		dev_err(smc->ldev, "T2 sleep: writing CLKL failed: %d\n", ret);
	else
		dev_info(smc->ldev, "T2 sleep: CLKL=all-FF (clock invalidated)\n");

	/* Disable OS watchdog (ui16, 2 bytes) */
	val_u16 = 0;
	ret = applesmc_write_key(smc, T2_KEY_OSWD, (u8 *)&val_u16, 2);
	if (ret)
		dev_err(smc->ldev, "T2 sleep: writing OSWD=0x0000 failed: %d\n", ret);
	else
		dev_info(smc->ldev, "T2 sleep: OSWD=0x0000 (watchdog disabled)\n");

	dev_info(smc->ldev, "T2 sleep: suspend_late complete\n");
	return 0;
}

/*
 * T2 resume: tell T2 the host is awake.
 * Runs as .resume_early, before device drivers resume.
 */
static void applesmc_wake_work_fn(struct work_struct *work)
{
	struct applesmc_device *smc = global_applesmc;
	u8 val_u8;

	if (!smc || !smc->reg.has_t2_sleep)
		return;

	dev_info(smc->ldev, "T2 wake: writing wake keys (deferred)\n");

	val_u8 = 0x00;
	if (applesmc_write_key(smc, T2_KEY_WKTP, &val_u8, 1))
		dev_err(smc->ldev, "T2 wake: WKTP=0x00 failed\n");

	/* NTOK=0x00 on wake — confirmed working on real T2 hardware */
	val_u8 = 0x00;
	if (applesmc_write_key(smc, T2_KEY_NTOK, &val_u8, 1))
		dev_err(smc->ldev, "T2 wake: NTOK=0x00 failed\n");

	dev_info(smc->ldev, "T2 wake: deferred wake keys complete\n");
}

static DECLARE_WORK(applesmc_wake_work, applesmc_wake_work_fn);

static int applesmc_pm_resume_early(struct device *dev)
{
	struct applesmc_device *smc = dev_get_drvdata(dev);
	...
	/* Defer SMC wake key writes — port I/O can hang if SMC isn't ready.
	 * Schedule to system workqueue so resume isn't blocked. */
	dev_info(smc->ldev, "T2 wake: scheduling deferred wake keys\n");
	schedule_work(&applesmc_wake_work);
	return 0;
}
...
static const struct dev_pm_ops applesmc_pm_ops = {
	.suspend = applesmc_pm_suspend_late,
	.resume_early = applesmc_pm_resume_early,
...
/*
 * Exported T2 sleep/wake helpers for apple-bce to call before/after 0x17.
 * These write the same SMC keys as suspend_late/resume_early.
 */
int applesmc_t2_prepare_sleep(void)
{
	struct applesmc_device *smc = global_applesmc;
	u8 val_u8;
	u8 clkl_buf[6];
	...
	pr_info("applesmc: t2_prepare_sleep called from apple-bce\n");
	...
	pr_info("applesmc: t2_prepare_sleep complete\n");
	return 0;
}
EXPORT_SYMBOL_GPL(applesmc_t2_prepare_sleep);

int applesmc_t2_prepare_wake(void)
{
	struct applesmc_device *smc = global_applesmc;
	u8 val_u8;
	int ret;
	...
	pr_info("applesmc: t2_prepare_wake called from apple-bce\n");
	...
	pr_info("applesmc: t2_prepare_wake complete\n");
	return 0;
}
EXPORT_SYMBOL_GPL(applesmc_t2_prepare_wake);

/* Standalone test trigger: write SMC sleep/wake keys without apple-bce.
 * echo 1 = write sleep keys (NTOK=0, WKTP=2, CLKL=FF, OSWD=0)
 * echo 2 = write wake keys (WKTP=0, NTOK=1)
 * echo 3 = write sleep keys, wait 10s for T2 response via SMC, then wake */
static int test_smc_sleep_set(const char *val, const struct kernel_param *kp)
{
	int cmd;
	if (kstrtoint(val, 10, &cmd))
		return -EINVAL;
	if (!global_applesmc) {
		pr_err("applesmc: test: no device\n");
		return -ENODEV;
	}
	if (cmd == 1) {
		pr_emerg("applesmc: TEST: writing sleep SMC keys\n");
		return applesmc_t2_prepare_sleep();
	} else if (cmd == 2) {
		pr_emerg("applesmc: TEST: writing wake SMC keys\n");
		return applesmc_t2_prepare_wake();
	} else if (cmd == 3) {
		int ret;
		pr_emerg("applesmc: TEST: sleep keys, wait 10s, wake keys\n");
		ret = applesmc_t2_prepare_sleep();
		if (ret) return ret;
		pr_emerg("applesmc: TEST: sleep keys written, waiting 10s...\n");
		msleep(10000);
		pr_emerg("applesmc: TEST: writing wake keys\n");
		ret = applesmc_t2_prepare_wake();
		pr_emerg("applesmc: TEST: done (wake ret=%d)\n", ret);
		return ret;
	}
	return -EINVAL;
}
static const struct kernel_param_ops test_smc_sleep_ops = {
	.set = test_smc_sleep_set,
};
module_param_cb(test_t2_sleep, &test_smc_sleep_ops, NULL, 0200);
MODULE_PARM_DESC(test_t2_sleep, "1=sleep keys, 2=wake keys, 3=sleep+wait+wake");
